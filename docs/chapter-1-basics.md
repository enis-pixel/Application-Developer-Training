...
For real world applications you would probably omit `--branch` for a more advanced default setup or customize it to your needs. See the [Moryx.Cli README](https://www.nuget.org/packages/Moryx.Cli#readme-body-tab) or `moryx --help` for more information.
:::
...
:::note
Starting it for the first time will restore NuGet packages. That can take a few minutes.
:::
...
:::note
When defining the hardness of pencils, the number (degree) is put first (`2B`, `2H`, etc.). Since this cannot be represented in code, it is switched for names, while everything else keeps the official format. More background is available in the [grading and classification article](https://en.wikipedia.org/wiki/Pencil#Grading_and_classification).
:::
...
:::note
The naming here comes from the fact that you would not necessarily add products here, but rather import them from other systems.
:::
...
:::note
Make sure to deselect all cells before adding more, so that they are added to the root level and not as children of other resources.
:::
